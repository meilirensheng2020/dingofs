...
class DumpFileTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    pathname_ = dirname_ + "/dingofs.dump";
...
TEST_F(DumpFileTest, MiscTest) {
  ...
  auto dumpfile = DumpFile(".dumpfile/dingofs.dump");
  ...
  dumpfile = DumpFile(".hello/dingofs.dump");
  ASSERT_EQ(dumpfile.Open(), DUMPFILE_ERROR::FAILED);

  dumpfile = DumpFile(".dumpfile/dingofs.dump");
