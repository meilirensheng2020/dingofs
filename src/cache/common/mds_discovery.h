...
#ifndef DINGOFS_SRC_CACHE_COMMON_MDS_DISCOVERY_H_
#define DINGOFS_SRC_CACHE_COMMON_MDS_DISCOVERY_H_
...
class MDSDiscovery;
using MDSDiscoveryUPtr = std::unique_ptr<MDSDiscovery>;

class MDSDiscovery {
 public:
  MDSDiscovery(RPCSPtr rpc) : rpc_(rpc){};
  ~MDSDiscovery() = default;

  static MDSDiscoveryUPtr New(RPCSPtr rpc) {
    return std::make_unique<MDSDiscovery>(rpc);
  }
...
#endif  // DINGOFS_SRC_CACHE_COMMON_MDS_DISCOVERY_H_
