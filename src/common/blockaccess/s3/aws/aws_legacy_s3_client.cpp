...
class LegacyLog : public LogSystemInterface {
 public:
  LegacyLog(LogLevel log_level) : log_level_(log_level) {}

  LogLevel GetLogLevel() const override { return log_level_; }

  void Log(LogLevel logLevel, const char* tag, const char* formatStr,
           ...) override {
    va_list args;
  ...
  void vaLog(LogLevel logLevel, const char* tag, const char* formatStr,
             va_list args) override {
    char message[kLogBufferSize] = {0};
    int ret = vsnprintf(message, kLogBufferSize, formatStr, args);
    if (ret < 0) {
      LOG(ERROR) << fmt::format("[s3_legacy.aws_sdk.{}] vsnprintf error.", tag);
      return;
    } else if (ret >= kLogBufferSize) {
      LOG(WARNING) << fmt::format(
          "[s3_legacy.aws_sdk.{}] vsnprintf truncated message, original "
          "size({}) > buffer size({}).",
          tag, ret, kLogBufferSize);
      message[kLogBufferSize - 1] = '\0';  // Ensure null termination
    }

    LogPrint(logLevel, tag, message);
  }

  void LogStream(LogLevel logLevel, const char* tag,
                 const Aws::OStringStream& messageStream) override {
    LogPrint(logLevel, tag, messageStream.str());
  }
 ...
 private:
  void LogPrint(LogLevel logLevel, const char* tag,
    ...
    switch (logLevel) {
      ...
      case LogLevel::Fatal:
        LOG(FATAL) << fmt::format("[s3_legacy.aws_sdk.{}] {}.", tag, message);
        break;
      case LogLevel::Error:
        LOG(ERROR) << fmt::format("[s3_legacy.aws_sdk.{}] {}.", tag, message);
        break;

      case LogLevel::Warn:
        LOG(WARNING) << fmt::format("[s3_legacy.aws_sdk.{}] {}.", tag, message);
        break;

      case LogLevel::Info:
      case LogLevel::Debug:
      case LogLevel::Trace:
        LOG(INFO) << fmt::format("[s3_legacy.aws_sdk.{}] {}.", tag, message);
        break;
      default:
        break;
    }
...
AwsLegacyS3Client::LogSystemInterfaceSPtr AwsLegacyS3Client::CreateLogger(
    int log_level) {
  return std::make_shared<LegacyLog>(static_cast<LogLevel>(log_level));
}
