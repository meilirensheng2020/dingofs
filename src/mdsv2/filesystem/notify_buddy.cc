...
void NotifyBuddy::SendMessage(uint64_t mds_id, BatchMessage& batch_message) {
  pb::mdsv2::NotifyBuddyRequest notify_message;
  ...
  for (auto& message : batch_message) {
    ...
    switch (message->type) {
      case Type::kRefreshFsInfo: {
        mut_message->set_type(pb::mdsv2::NotifyBuddyRequest::TYPE_REFRESH_FS_INFO);
      ...
      case Type::kRefreshInode: {
        mut_message->set_type(pb::mdsv2::NotifyBuddyRequest::TYPE_REFRESH_INODE);
