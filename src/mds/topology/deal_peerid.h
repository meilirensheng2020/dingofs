 ...
 * Author: wanghai01
 */
#ifndef DINGOFS_SRC_MDS_TOPOLOGY_DEAL_PEERID_H_
#define DINGOFS_SRC_MDS_TOPOLOGY_DEAL_PEERID_H_
...
inline bool SplitAddrToIpPort(const std::string& addr, std::string* ipstr,
  ...
  dingofs::utils::SplitString(addr, ":", &items);
  if (2 == items.size() && dingofs::utils::StringToUl(items[1], port)) {
    *ipstr = items[0];
    return true;
  }
  return false;
...
#endif  // DINGOFS_SRC_MDS_TOPOLOGY_DEAL_PEERID_H_
