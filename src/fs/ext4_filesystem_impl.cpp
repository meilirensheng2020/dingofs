...
bool Ext4FileSystemImpl::CheckKernelVersion() {
  ...
  ::dingofs::utils::SplitString(kernel_info.release, "-", &elements);
  if (elements.size() == 0) {
  ...
  ::dingofs::utils::SplitString(elements[0], ".", &numbers);
...
int Ext4FileSystemImpl::Mkdir(const string& dirName) {
  vector<string> names;
  ::dingofs::utils::SplitString(dirName, "/", &names);
...
int Ext4FileSystemImpl::Read(int fd, char* buf, uint64_t offset, int length) {
  ...
  while (remainLength > 0) {
    ...
    if (ret == 0) {
      LOG(WARNING) << "pread returns zero." << "offset: " << offset
                   << ", length: " << remainLength;
      break;
    }
    if (ret < 0) {
