 ...
 */
inline int GetFuseFd(const char* socket_path, void* data_buf,
  ...
  if (client_fd == -1) {
    perror("socket");
    LOG(ERROR) << "create socket failed, error: " << std::strerror(errno);
    return -1;
  }
  ...
  if (connect(client_fd, (struct sockaddr*)&addr, sizeof(addr)) == -1) {
    LOG(ERROR) << "connect uds server failed, error: " << std::strerror(errno);
    return -1;
  }
