...
struct WarmupTaskContext {
  ...
  WarmupType type;
  Ino task_key;
  // comma separated inodeid('1000023,10000021,...'), provide by dingo-tools
...
class WarmupTask {
 public:
  ...
  uint64_t GetTotal() const {
    return progress_.total.load(std::memory_order_relaxed);
  };
  uint64_t GetFinished() const {
    return progress_.finish.load(std::memory_order_relaxed);
  };
  uint64_t GetErrors() const {
    return progress_.errors.load(std::memory_order_relaxed);
  };

 private:
  struct WarmupProgress {
    std::atomic<uint64_t> total;
    std::atomic<uint64_t> finish;
