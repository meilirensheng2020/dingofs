...
#include "client/common/constant.h"
...
Status VFSImpl::Ioctl(ContextSPtr ctx, Ino ino, uint32_t uid, unsigned int cmd,
  ...
  if (DINGO_UNLIKELY(IsInternalNode(ino))) {
  ...
  //         fmt::format("ioctl cmd({}) not supported", cmd));
  // }
  ...
  if (op_code == 1) {  // set
    ...
    // TODO: FS_NOATIME_FL iflag from fuse is 0xffffff80
    // does't match FS_NOATIME_FL(0x00000080)?
    // attr.flags = (iflag & FS_NOATIME_FL) ? (attr.flags | kFlagNoAtime)
    //                                      : (attr.flags & ~kFlagNoAtime);
    ...
    if (((cmd >> 8) & 0xFF) == 'f') {  // FS_IOC_GETFLAGS
      ...
      if (out_bufsz == 8) {
        utils::EncodeNativeEndian(out_buf, iflag);
      } else if (out_bufsz == 4) {
        utils::EncodeNativeEndian_uint32(out_buf, uint32_t(iflag));
      } else {
        return Status::InvalidParam(
            fmt::format("out_bufsz({}) not supported", out_bufsz));
      }
