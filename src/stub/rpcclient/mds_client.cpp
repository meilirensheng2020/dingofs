...
static CacheGroupErrCode ToCacheGroupErrCode(int err) {
  if (err < 0) {
    return CacheGroupErrCode::CacheGroupErrNetError;
  }
...
pb::mds::cachegroup::CacheGroupErrCode MdsClientImpl::JoinCacheGroup(
  ...
  auto task = RPCTask {
    ...
    mdsbasecli_->JoinCacheGroup(group_name, ip, port, weight, &response, cntl,
                                channel);
...
pb::mds::cachegroup::CacheGroupErrCode MdsClientImpl::ListCacheGroupMembers(
  ...
  auto task = RPCTask {
    ...
    mdsbasecli_->ListCacheGroupMembers(group_name, &response, cntl, channel);

    CHECK_RPC_AND_RETRY_IF_ERROR("ListCacheGroupMembers");
    ...
    if (status != CacheGroupErrCode::CacheGroupOk) {
      ...
      VLOG(9) << "ListCacheGroupMembers, response: "
