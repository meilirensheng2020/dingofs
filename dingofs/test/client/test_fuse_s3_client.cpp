...
#include "dingofs/test/client/mock_client_s3_adaptor.h"
...
TEST_F(TestFuseS3Client, test_Init_with_KVCache) {
  ...
  // test init kvcache success
  {
    ...
    ASSERT_EQ(DINGOFS_ERROR::OK, testclient->Init(opt));
  ...
  // test init kvcache fail
  {
    ...
    ASSERT_EQ(DINGOFS_ERROR::INTERNAL, testclient->Init(opt));
    testclient->UnInit();
  }
  dingofs::client::common::FLAGS_supportKVcache = false;
}
...
TEST_F(TestFuseS3Client, test_Init_with_cache_size_0) {
  ...
  ASSERT_EQ(DINGOFS_ERROR::CACHETOOSMALL, testClient->Init(opt));
  testClient->UnInit();
}
...
TEST_F(TestFuseS3Client, warmUp_inodeBadFd) {
  ...
  EXPECT_CALL(*inodeManager_, GetInode(_, _))
      .WillOnce(
          DoAll(SetArgReferee<1>(inodeWrapper), Return(DINGOFS_ERROR::BAD_FD)));
  auto old = client_->GetFsInfo()->fstype();
  client_->GetFsInfo()->set_fstype(FSType::TYPE_S3);
  client_->PutWarmFilelistTask(
      inodeid,
      dingofs::client::common::WarmupStorageType::kWarmupStorageTypeDisk);
...
TEST_F(TestFuseS3Client, warmUp_Warmfile_error_GetDentry01) {
  ...
  EXPECT_CALL(*dentryManager_, GetDentry(_, _, _))
      .WillOnce(DoAll(SetArgPointee<2>(dentry), Return(DINGOFS_ERROR::BAD_FD)));
  EXPECT_CALL(*inodeManager_, GetInode(_, _))
      .WillOnce(
...
TEST_F(TestFuseS3Client, warmUp_Warmfile_error_GetDentry02) {
  ...
  EXPECT_CALL(*dentryManager_, GetDentry(_, _, _))
      .WillOnce(
          DoAll(SetArgPointee<2>(dentry), Return(DINGOFS_ERROR::NOTEXIST)));
  EXPECT_CALL(*inodeManager_, GetInode(_, _))
      .WillOnce(
          DoAll(SetArgReferee<1>(inodeWrapper), Return(DINGOFS_ERROR::OK)))
      .WillOnce(
...
TEST_F(TestFuseS3Client, warmUp_fetchDataEnqueue__error_getinode) {
  ...
  auto inodeWrapper = std::make_shared<InodeWrapper>(inode, metaClient_);
  EXPECT_CALL(*dentryManager_, GetDentry(_, _, _))
      .WillOnce(DoAll(SetArgPointee<2>(dentry), Return(DINGOFS_ERROR::OK)));
  EXPECT_CALL(*inodeManager_, GetInode(_, _))
      .WillOnce(
          DoAll(SetArgReferee<1>(inodeWrapper), Return(DINGOFS_ERROR::OK)))
      .WillOnce(
...
TEST_F(TestFuseS3Client, warmUp_FetchDentry_TYPE_SYM_LINK) {
  ...
  auto inodeWrapper = std::make_shared<InodeWrapper>(inode, metaClient_);
  EXPECT_CALL(*dentryManager_, GetDentry(_, _, _))
      .WillOnce(DoAll(SetArgPointee<2>(dentry1), Return(DINGOFS_ERROR::OK)));
  EXPECT_CALL(*inodeManager_, GetInode(_, _))
      .WillOnce(
          DoAll(SetArgReferee<1>(inodeWrapper), Return(DINGOFS_ERROR::OK)))
      .WillOnce(
...
TEST_F(TestFuseS3Client, warmUp_FetchChildDentry_error_ListDentry) {
  ...
  EXPECT_CALL(*dentryManager_, ListDentry(_, _, _, _, _))
      .WillOnce(
          DoAll(SetArgPointee<1>(dlist), Return(DINGOFS_ERROR::NOTEXIST)));
...
TEST_F(TestFuseS3Client, warmUp_FetchChildDentry_suc_ListDentry) {
  ...
  EXPECT_CALL(*dentryManager_, ListDentry(_, _, _, _, _))
      .WillOnce(DoAll(SetArgPointee<1>(dlist), Return(DINGOFS_ERROR::OK)))
      .WillOnce(
          DoAll(SetArgPointee<1>(dlist), Return(DINGOFS_ERROR::NOTEXIST)));
...
TEST_F(TestFuseS3Client, FuseInit_when_fs_exist) {
  ...
  DINGOFS_ERROR ret = client_->SetMountStatus(&mOpts);
...
TEST_F(TestFuseS3Client, FuseOpWriteFailed) {
  ...
  DINGOFS_ERROR ret =
      client_->FuseOpWrite(req, ino, buf, size, off, &fi, &fileOut);
  ASSERT_EQ(DINGOFS_ERROR::INTERNAL, ret);
...
TEST_F(TestFuseS3Client, FuseOpFsync) {
  ...
  DINGOFS_ERROR ret = client_->FuseOpFsync(req, ino, 0, fi);
  ASSERT_EQ(DINGOFS_ERROR::OK, ret);
...
TEST_F(TestFuseS3Client, FuseOpFlush) {
  ...
  ASSERT_EQ(DINGOFS_ERROR::UNKNOWN, client_->FuseOpFlush(req, ino, fi));

  LOG(INFO) << "############ case2: test disable cto and flush ok";
  EXPECT_CALL(*s3ClientAdaptor_, Flush(ino))
      .WillOnce(Return(DINGOFS_ERROR::OK));
  ASSERT_EQ(DINGOFS_ERROR::OK, client_->FuseOpFlush(req, ino, fi));
...
TEST_F(TestFuseS3Client, FuseOpGetXattr_NotSummaryInfo) {
  ...
  DINGOFS_ERROR ret = client_->FuseOpGetXattr(req, ino, name, &value, size);
  ASSERT_EQ(DINGOFS_ERROR::NODATA, ret);
}
...
TEST_F(TestFuseS3Client, FuseOpGetXattr_NotEnableSumInDir) {
  ...
  fuse_req_t req = nullptr;
  fuse_ino_t ino = 1;
  const char rname[] = "dingo.dir.rfbytes";
  const char name[] = "dingo.dir.fbytes";
  size_t size = 100;
  std::string value;
  ...
  EXPECT_CALL(*inodeManager_, GetInodeAttr(ino, _))
      .WillOnce(DoAll(SetArgPointee<1>(inode), Return(DINGOFS_ERROR::OK)));
  EXPECT_CALL(*dentryManager_, ListDentry(_, _, _, _, _))
      .WillOnce(DoAll(SetArgPointee<1>(dlist), Return(DINGOFS_ERROR::OK)))
      .WillOnce(DoAll(SetArgPointee<1>(dlist1), Return(DINGOFS_ERROR::OK)));
  EXPECT_CALL(*inodeManager_, BatchGetInodeAttr(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(attrs), Return(DINGOFS_ERROR::OK)))
      .WillOnce(DoAll(SetArgPointee<1>(attrs1), Return(DINGOFS_ERROR::OK)));

  DINGOFS_ERROR ret = client_->FuseOpGetXattr(req, ino, rname, &value, size);
  ASSERT_EQ(DINGOFS_ERROR::OK, ret);
...
TEST_F(TestFuseS3Client, FuseOpGetXattr_NotEnableSumInDir_Failed) {
  ...
  EXPECT_CALL(*inodeManager_, GetInodeAttr(ino, _))
      .WillOnce(
          DoAll(SetArgPointee<1>(inode), Return(DINGOFS_ERROR::INTERNAL)));
  DINGOFS_ERROR ret = client_->FuseOpGetXattr(req, ino, rname, &value, size);
  ASSERT_EQ(DINGOFS_ERROR::INTERNAL, ret);
...
TEST_F(TestFuseS3Client, FuseOpGetXattr_EnableSumInDir) {
  ...
  fuse_req_t req = nullptr;
  fuse_ino_t ino = 1;
  const char name[] = "dingo.dir.rentries";
  size_t size = 100;
  std::string value;
  ...
  EXPECT_CALL(*inodeManager_, GetInodeAttr(ino, _))
      ...
      .WillOnce(DoAll(SetArgPointee<1>(inode), Return(DINGOFS_ERROR::OK)))
      .WillOnce(DoAll(SetArgPointee<1>(inode), Return(DINGOFS_ERROR::OK)))
      .WillOnce(DoAll(SetArgPointee<1>(attr), Return(DINGOFS_ERROR::OK)));
  EXPECT_CALL(*dentryManager_, ListDentry(_, _, _, _, _))
      .WillOnce(DoAll(SetArgPointee<1>(dlist), Return(DINGOFS_ERROR::OK)))
      .WillOnce(DoAll(SetArgPointee<1>(emptyDlist), Return(DINGOFS_ERROR::OK)));
  EXPECT_CALL(*inodeManager_, BatchGetXAttr(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(xattrs), Return(DINGOFS_ERROR::OK)));
...
TEST_F(TestFuseS3Client, FuseOpGetXattr_EnableSumInDir_Failed) {
  ...
  fuse_req_t req = nullptr;
  fuse_ino_t ino = 1;
  const char name[] = "dingo.dir.entries";
  const char rname[] = "dingo.dir.rentries";
  size_t size = 100;
  std::string value;
  ...
  EXPECT_CALL(*inodeManager_, GetInodeAttr(ino, _))
      .Times(2)
      .WillRepeatedly(
          DoAll(SetArgPointee<1>(inode), Return(DINGOFS_ERROR::OK)));
  EXPECT_CALL(*dentryManager_, ListDentry(_, _, _, _, _))
      .WillOnce(Return(DINGOFS_ERROR::NOTEXIST));
  ret = client_->FuseOpGetXattr(req, ino, rname, &value, size);
  ASSERT_EQ(DINGOFS_ERROR::INTERNAL, ret);
  ...
  EXPECT_CALL(*dentryManager_, ListDentry(_, _, _, _, _))
      .WillOnce(DoAll(SetArgPointee<1>(dlist), Return(DINGOFS_ERROR::OK)))
      .WillRepeatedly(
          DoAll(SetArgPointee<1>(emptyDlist), Return(DINGOFS_ERROR::OK)));
  EXPECT_CALL(*inodeManager_, BatchGetXAttr(_, _))
      .WillOnce(Return(DINGOFS_ERROR::INTERNAL));
  ret = client_->FuseOpGetXattr(req, ino, rname, &value, size);
  ASSERT_EQ(DINGOFS_ERROR::INTERNAL, ret);
...
TEST_F(TestFuseS3Client, FuseOpLink_EnableSummary) {
  ...
  EXPECT_CALL(*inodeManager_, GetInode(_, _))
      .WillOnce(
          DoAll(SetArgReferee<1>(inodeWrapper), Return(DINGOFS_ERROR::OK)))
      .WillRepeatedly(
          DoAll(SetArgReferee<1>(pinodeWrapper), Return(DINGOFS_ERROR::OK)));
...
TEST_F(TestFuseS3Client, FuseOpUnlink_EnableSummary) {
  ...
  DINGOFS_ERROR ret = client_->FuseOpUnlink(req, parent, name.c_str());
  ASSERT_EQ(DINGOFS_ERROR::OK, ret);
...
TEST_F(TestFuseS3Client, FuseOpOpen_Trunc_EnableSummary) {
  ...
  FileOut fileOut;
  DINGOFS_ERROR ret = client_->FuseOpOpen(req, ino, &fi, &fileOut);
  ASSERT_EQ(DINGOFS_ERROR::OK, ret);
...
TEST_F(TestFuseS3Client, FuseOpListXattr) {
  ...
  EXPECT_CALL(*inodeManager_, GetInodeAttr(ino, _))
      .WillOnce(
          DoAll(SetArgPointee<1>(inode), Return(DINGOFS_ERROR::INTERNAL)));
  DINGOFS_ERROR ret = client_->FuseOpListXattr(req, ino, buf, size, &realSize);
  ASSERT_EQ(DINGOFS_ERROR::INTERNAL, ret);
  ...
  ASSERT_EQ(DINGOFS_ERROR::OUT_OF_RANGE, ret);
...
TEST_F(TestFuseS3Client, FuseOpSetXattr_TooLong) {
  ...
  DINGOFS_ERROR ret = client_->FuseOpSetXattr(req, ino, name, value, size, 0);
  ASSERT_EQ(DINGOFS_ERROR::OUT_OF_RANGE, ret);
}
...
TEST_F(TestFuseS3Client, FuseOpWriteQosTest) {
  dingofs::client::common::FLAGS_fuseClientAvgWriteBytes = 100;
  dingofs::client::common::FLAGS_fuseClientBurstWriteBytes = 150;
  dingofs::client::common::FLAGS_fuseClientBurstWriteBytesSecs = 180;
...
TEST_F(TestFuseS3Client, FuseOpReadQosTest) {
  dingofs::client::common::FLAGS_fuseClientAvgReadBytes = 100;
  dingofs::client::common::FLAGS_fuseClientBurstReadBytes = 150;
  dingofs::client::common::FLAGS_fuseClientBurstReadBytesSecs = 180;
