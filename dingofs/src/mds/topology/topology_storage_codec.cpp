...
using dingofs::mds::FS2MEMCACHECLUSTERKEYPREFIX;
using dingofs::mds::MEMCACHECLUSTERKEYEND;
using dingofs::mds::MEMCACHECLUSTERKEYPREFIX;
...
bool TopologyStorageCodec::DecodeFs2MemcacheClusterKey(const std::string& value,
                                                       FsIdType* data) {
  size_t prefixLen = TOPOLOGY_PREFIX_LENGTH;
  *data = dingofs::utils::DecodeBigEndian(&(value[prefixLen]));
  return true;
}
